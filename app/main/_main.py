...
def init_app(app):
    ...
    main_bp.static_url_path = get_app_config('APP_PAGE_STATIC_URL_PATH') or '/'
    page.page_mapping.update(get_app_config('APP_PAGE_MAPPING') or {})

    # disable cache
    if get_app_config('APP_PAGE_CACHE_ENABLED') is not True:
...
def init_app_security(app):
    ...
    def _init_security(response):
        ...
        headers['Server'] = ''  # 清空
        headers["X-Frame-Options"] = 'SAMEORIGIN'  # 防点击劫持
        # headers['Content-Security-Policy'] = "default-src 'self'"
        headers['Content-Security-Policy'] = (
            "default-src 'self';"  # 只允许加载同源资源
            " script-src 'self' 'unsafe-eval';"  # 只允许加载同源 JS，允许使用 eval()
            " style-src 'self' 'unsafe-inline';"  # 只允许加载同源 CSS，允许内联样式
            " img-src 'self' data:;"  # 图片只能来自同源或 data: UR
            " base-uri 'self';"  # 限制 <base> 标签只能指向同源
            " frame-ancestors 'self';"  # 限制 iframe 嵌入
        )
        headers['Permissions-Policy'] = 'geolocation=(), camera=()'  # 控制浏览器 API 权限, 页面禁用访问地理地理位置+摄像头
...
from . import page, main_bp, guide, errors

if guide or page or main_bp or errors:
    pass
